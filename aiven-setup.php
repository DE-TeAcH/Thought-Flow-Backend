...
// All your blog data - prepared statements will handle apostrophes safely
$blogs = [
...
// Use prepared statements - they automatically escape special characters
$stmt = $conn->prepare("INSERT INTO blogs (title, author, excerpt, content, category, read_time) VALUES (?, ?, ?, ?, ?, ?)");
...
foreach ($blogs as $blog) {
    $stmt->bind_param("sssssi", $blog[0], $blog[1], $blog[2], $blog[3], $blog[4], $blog[5]);
    if ($stmt->execute()) {
        $inserted++;
    } else {
        echo "❌ Error inserting blog: " . $stmt->error . "<br>";
    }
}

$stmt->close();

echo "✅ Inserted $inserted blog posts successfully!<br><br>";
echo "<strong>🎉 Database setup complete!</strong><br>";
echo "Now delete this file and test: https://thought-flow-hh8a.onrender.com/homeBlogs.php";
